...
pub enum Command {
    ...
    Logout,
    /// Connect a model or subscription (opens the picker in the TUI; `--xai` for the optional xAI account)
    Login {
        /// Ignored (kept for backwards compatibility). OAuth2 is now the only auth method.
        #[arg(long, hide = true)]
        legacy: bool,
        /// Sign in with an xAI account (optional; opens auth.x.ai). Records the opt-in in $WORKSHOP_HOME.
        #[arg(long = "xai")]
        xai: bool,
        /// Use the browser (loopback) OAuth transport for the configured sign-in provider.
