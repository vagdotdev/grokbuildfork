...
fn dispatch_confirm_reset_setting_reset_dispatches_typed_setter_for_shared_enum() {
    ...
    with_theme_test_env(|| {
        ...
        // Reset dispatches SetTheme("oscura-midnight"), the registered default
        assert_eq!(effects.len(), 1);
        match effects.first() {
            Some(Effect::PersistSetting { key, value, .. }) => {
                assert_eq!(*key, "theme");
                assert_eq!(value, &SettingValue::Enum("oscura-midnight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("oscura-midnight"));
    });
}
