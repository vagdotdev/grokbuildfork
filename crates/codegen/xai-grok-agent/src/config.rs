...
/// `run_terminal_command` is omitted so exploration cannot mutate the workspace — the toolset enforces read-only, not just the prompt.
/// With no `BashTool`, the background-task helpers are also omitted.
