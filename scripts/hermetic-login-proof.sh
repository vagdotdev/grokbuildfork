#!/usr/bin/env bash
# Runtime proof for milestone A/B: run the built `workshop` binary in a hermetic
# sandbox and log every DNS query, every connect(), and every open of a decoy
# foreign-credential file. Run it *inside a terminal* (tmux pane or a terminal
# emulator): the TUI takes over the PTY, you drive Login / `l` / `/login`, and
# on exit the script writes <outdir>/egress-proof.log with a verdict.
#
...

{
  ...
  echo
  echo "## connect() to loopback (DNS stub on 127.0.0.1:53 and local sockets)"
